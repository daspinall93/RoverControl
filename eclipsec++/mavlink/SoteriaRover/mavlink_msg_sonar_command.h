...
#define MAVLINK_MSG_ID_SONAR_COMMAND_CRC 142
#define MAVLINK_MSG_ID_26_CRC 142
...
#define MAVLINK_MESSAGE_INFO_SONAR_COMMAND { \
    ...
    "SONAR_COMMAND", \
    1, \
    {  { "new_command", NULL, MAVLINK_TYPE_UINT8_T, 0, 0, offsetof(mavlink_sonar_command_t, new_command) }, \
         } \
}
#else
#define MAVLINK_MESSAGE_INFO_SONAR_COMMAND { \
...
{
    return mavlink_msg_sonar_command_pack(system_id, component_id, msg, sonar_command->new_command);
}
...
{
    return mavlink_msg_sonar_command_pack_chan(system_id, component_id, chan, msg, sonar_command->new_command);
}
...
static inline void mavlink_msg_sonar_command_send(mavlink_channel_t chan, uint8_t new_command)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_SONAR_COMMAND_LEN];
    _mav_put_uint8_t(buf, 0, new_command);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_sonar_command_send(chan, sonar_command->new_command);
#else
 ...
 */
static inline void mavlink_msg_sonar_command_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint8_t new_command)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
/**
 * @brief Get field new_command from sonar_command message
 *
 * @return 
 */
static inline uint8_t mavlink_msg_sonar_command_get_new_command(const mavlink_message_t* msg)
{
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    sonar_command->new_command = mavlink_msg_sonar_command_get_new_command(msg);
#else
